...
test('SkewHeap', t => {
  t.equal(typeof SkewHeap, 'function');
  ...
  {
    const heap = SkewHeap.from([3, 2, 5, 1, 4]);
    const array = [];
  ...
  {
    const heap = SkewHeap.from([3, 2, 5, 1, 4], {less: (a, b) => a > b});
    const array = [];
    while (!heap.isEmpty) array.push(heap.pop());
  ...
  {
    const heap = new SkewHeap().push(3).push(2).push(5).push(1).push(4);
    const array = [];
    while (!heap.isEmpty) array.push(heap.pop());
    t.deepEqual(array, [1, 2, 3, 4, 5]);
  }
  ...
  {
    const heap = SkewHeap.from(['b', 'c', 'a', 'b'], {compare: (a, b) => a.localeCompare(b)});
    const array = [];
    while (!heap.isEmpty) array.push(heap.pop());
  ...
  heap.merge(SkewHeap.from([2, 1, 3], heap));
  ...
  const heap2 = SkewHeap.from([3, 2, 5, 1, 4]);
