...
2. Create `src/<name>.d.ts` (or `src/<category>/<name>.d.ts`) with TypeScript declarations.
...
5. If the module needs a short import path (e.g., `list-toolkit/<name>`), create a thin re-export module at `src/<name>.js` and add an entry to `exports` in `package.json`.
6. Create a wiki page following the existing naming conventions:
...
7. Add a link to the new wiki page in `wiki/Home.md` (both the main section and the "Direct links" sections).

## Documentation updates

8. Update `ARCHITECTURE.md` — add the module to the project layout tree and dependency graph if applicable.
9. Update `llms.txt` with a brief description of the new module.
10. Update `llms-full.txt` with the full API reference for the new module.
11. Update `AGENTS.md` if the module changes the architecture quick reference.
